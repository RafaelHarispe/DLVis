...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
            #weight_scale = np.sqrt(2/50)  # Para probar inicializacion He
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        cache_aff, cache_bn, cache_relu, cache_drop = {}, {}, {}, {} 
        x = X
        for layer in range(1, self.num_layers):
            ...
            if self.use_dropout:
                x, cache_drop[layer] = dropout_forward(x, self.dropout_param)

        W, b = 'W'+str(self.num_layers), 'b'+str(self.num_layers)
        scores, cache_aff[self.num_layers] = affine_forward(x, self.params[W], self.params[b])
        ...
        W, b = 'W'+str(self.num_layers), 'b'+str(self.num_layers)
        Wi = self.params[W]
        dhidden, grads[W], grads[b] = affine_backward(dscores, cache_aff[self.num_layers])
        grads[W] += reg * Wi

        sum_W_sqr = np.sum(Wi * Wi)
        ...
        for layer in range(self.num_layers-1, 0, -1):
            ...
            if self.use_batchnorm:
                dhidden, grads[gamma], grads[beta] = batchnorm_backward(dhidden, cache_bn[layer])
            dhidden, grads[W], grads[b] = affine_backward(dhidden, cache_aff[layer])
 
            grads[W] += reg * Wi

            sum_W_sqr += np.sum(Wi * Wi)

        loss += 0.5 * reg * sum_W_sqr
